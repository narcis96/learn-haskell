...
onlyDigit a = length [ch | ch<-a, isDigit ch] == length a
...
getNoStr [] = []
getNoStr (x:xs) | isDigit x = x : (getNoStr xs)
		| otherwise = []
...
getNo :: String -> Int
getNo str = foldl (\res digit-> res*10 + digitToInt digit) 0 str
...
removeNotDigit str = filter isDigit str
...
parseTerm str | plus = x
	      | otherwise= (-1)*x
	      where plus = even (length (takeWhile (=='-') str))
		    x = getNo (getNoStr (dropWhile (=='-') str))
...
splitWhenPlus str = splitWhen (=='+') str
...
takeWhile2 _ [] = []
takeWhile2 pred (x:xs) | pred x = (x : takeWhile2 pred xs)
		       | otherwise = []
drop2 k [] = []
drop2 k li = [li!!i| i <- [k..(length li)-1]]

dropWhileMy _ [] = []
dropWhileMy pred (x:xs) | (pred x) = (dropWhileMy pred xs)
		        | otherwise = (x:xs)

termList :: String ->[String]
termList [] = [[]]
termList (x:xs) = [he] ++ termList ta 
		where str = (x:xs)
		      he = takeWhile2(/='+') str
		      ta = drop2 1 (dropWhileMy (/='+') str)
...
eval :: String -> Int
eval str = sum (map parseTerm (splitWhenPlus str))


main = do
	print (onlyDigit "123")
	print (onlyDigit "12a3")
	
	print (getNoStr "12ab1") -- "12"
	print (getNoStr "-12") -- ""
	print (getNoStr "-ab") -- ""

	print (getNo "123")
	print (removeNotDigit "bo1s2s3")

	print (parseTerm "12+3") -- 12
	print (parseTerm "+3") -- 0
	print (parseTerm "--3") -- 3

	print (splitWhenPlus "-12 a +cd-1+--34ab") -- ["-12 a ", "cd-1", "--34ab"]
	print (splitWhenPlus "++12+cd-1+") --["","","12","cd-1",""])

	print (termList "-12 a +cd-1+--34ab") -- ["-12 a ", "cd-1", "--34ab"]
	print (termList "++12+cd-1+") --["","","12","cd-1",""])


	print (eval "-12 a +cd-1+--34ab") --22
